...
const createUI = () => {
    ...
    const toggleWindowMode = (panelId) => {
        update((state) => {
    ...
    const moveWindow = (panelId, dx, dy) => {
        update((state) => {
            state.windows = state.windows.map((w) => w.panelId === panelId ? { ...w, x: w.x + dx, y: w.y + dy } : w);
            return { ...state };
        });
    };
    const resizeWindow = (panelId, dx, dy, minWidth = 240, minHeight = 160) => {
        update((state) => {
            state.windows = state.windows.map((w) => {
                if (w.panelId !== panelId)
                    return w;
                return {
                    ...w,
    ...
    const focusWindow = (panelId) => {
        update((state) => {
            state.nextZ += 1;
            state.windows = state.windows.map((w) => w.panelId === panelId ? { ...w, z: state.nextZ } : w);
            return { ...state };
        });
    };
    const dockPanel = (panelId, zone) => {
        update((state) => {
            state.windows = state.windows.filter((w) => w.panelId !== panelId);
